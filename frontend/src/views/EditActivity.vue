  ...
  <div class="createActivity">
    ...
    <div>
      <input
        ...
        class="input"
        v-model="name"
        placeholder="Enter the App update Name "
      />
    </div>
    ...
    <div>
      <input
        ...
        class="input"
        v-model="password"
        placeholder="Enter the update App password "
      />
    </div>
    <button class="btn" v-on:click="Update">Update Activity</button>
    <transition name="alert-in">
            <h3 v-if="afterUpdate" class="alert">your Password with id: {{ id }} is Updated.</h3>
  ...
  methods: {
    async Update() {
      ...
      const response = await fetch(
        "http://localhost:3000/applications/" + this.id,
        requestOptions
      );
